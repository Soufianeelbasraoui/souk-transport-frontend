...
function CreerCargaison() {
  ...
  useEffect(() => {
    ...
    const chargerExpediteurs = async () => {
      try {
        const response = await api.get("/api/users/filter/role", {
          params: {
            role: "EXPEDITEUR",
          },
        });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isAdmin) {
        payload.expediteurId = data.expediteurId;
      }
      ...
      setTimeout(() => {
        navigate(
          isAdmin ? "/admin/cargaisons": "/expediteur/cargaisons" );
      }, 1500);
    } catch (error) {
      ...
      <main className="main-content">
        <div className="page-header">
          <div>
            ...
            <p>
              Remplissez les informations pour enregistrer votre
              cargaison.
            </p>
          </div>
